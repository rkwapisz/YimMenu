...
namespace big
{
	...
	bool world_to_screen::w2s(const rage::fvector3 entity_position, rage::fvector2& screen)
	{
		...
		tVec.x =
			(vRight.x * entity_position.x) + (vRight.y * entity_position.y) + (vRight.z * entity_position.z) + vRight.w;
		tVec.y = (vUp.x * entity_position.x) + (vUp.y * entity_position.y) + (vUp.z * entity_position.z) + vUp.w;
		tVec.z = (vForward.x * entity_position.x) + (vForward.y * entity_position.y) + (vForward.z * entity_position.z)
			+ vForward.w;
		...
		rage::fvector2 resolution = {static_cast<float>(*g_pointers->m_gta.m_resolution_x),
